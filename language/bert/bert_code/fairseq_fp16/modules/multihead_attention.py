...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    #norm_acti = F.linear(query, in_proj_weight, in_proj_bias)
    if(ma_module.is_training):
        ...
        if(ma_module.need_decompose):
            ...
            in_left_data, in_right_data, in_residual_data = weight_decomposition(in_proj_weight.data, rank=args.rank, mode=args.mode)
            out_left_data, out_right_data, out_residual_data = weight_decomposition(out_proj_weight.data, rank=args.rank, mode=args.mode)
        ...
        #print('attn in normal acti norm: ', norm_acti.norm().item(), 'lrk acti norm: ', acti.norm().item())
    else:
    ...
    #norm_acti = F.linear(attn_output, ma_module.out_cached, out_proj_bias)
    if(ma_module.is_training):
        ...
        attn_output = residual_acti1+lrk_acti1#F.linear(attn_output, out_proj_weight, out_proj_bias)
        #print('attn out, normal acti norm: ', norm_acti.norm().item(), 'lrk acti norm: ', attn_output.norm().item())
    else:
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, args, dropout=0., bias=True,
        ...
        self.attn_head_dim = args.attn_head_dim
        if(self.attn_head_dim <= 0):
            self.head_dim = embed_dim // num_heads
            self.attn_embed_dim = embed_dim
        else:
            self.head_dim = self.attn_head_dim
            self.attn_embed_dim = self.head_dim * num_heads
    ...
    def use_batch_grad(self, scale=None):
        if(self.in_proj_left.weight.grad == None):
            self.in_proj_left.weight.grad = process_batch_grad(self.in_proj_left.weight.batch_grad, scale=scale)#torch.sum(self.in_proj_left.weight.batch_grad, dim=0)
            self.in_proj_right.weight.grad = process_batch_grad(self.in_proj_right.weight.batch_grad, scale=scale)#torch.sum(self.in_proj_right.weight.batch_grad, dim=0)
            self.out_proj_left.weight.grad = process_batch_grad(self.out_proj_left.weight.batch_grad, scale=scale)#torch.sum(self.out_proj_left.weight.batch_grad, dim=0)
            self.out_proj_right.weight.grad = process_batch_grad(self.out_proj_right.weight.batch_grad, scale=scale)#torch.sum(self.out_proj_right.weight.batch_grad, dim=0)

            #print(torch.norm(self.in_proj_left.weight.grad).cpu().numpy())
        else:
            self.in_proj_left.weight.grad += process_batch_grad(self.in_proj_left.weight.batch_grad, scale=scale)#torch.sum(self.in_proj_left.weight.batch_grad, dim=0)
            self.in_proj_right.weight.grad += process_batch_grad(self.in_proj_right.weight.batch_grad, scale=scale)#torch.sum(self.in_proj_right.weight.batch_grad, dim=0)
            self.out_proj_left.weight.grad += process_batch_grad(self.out_proj_left.weight.batch_grad, scale=scale)#torch.sum(self.out_proj_left.weight.batch_grad, dim=0)
            self.out_proj_right.weight.grad += process_batch_grad(self.out_proj_right.weight.batch_grad, scale=scale)#torch.sum(self.out_proj_right.weight.batch_grad, dim=0)            
            #print(torch.norm(self.in_proj_left.weight.grad.view()).cpu().numpy())
    def _assign_full_grad(self, left, right, host, linear_eval=False):
        #host.grad = None
        if(not linear_eval):
    ...
    def assign_full_grad(self, linear_eval=False):
        ## assign grad for
        ...
        self._assign_full_grad(self.in_proj_left.weight, self.in_proj_right.weight, self.in_proj_weight, linear_eval=False)
        self._assign_full_grad(self.out_proj_left.weight, self.out_proj_right.weight, self.out_proj.weight, linear_eval=False)

        self.in_proj_left.weight.grad = None#.mul_(0.)
        self.in_proj_right.weight.grad = None#.mul_(0.)
        self.out_proj_left.weight.grad = None#.mul_(0.)
        self.out_proj_right.weight.grad = None#.mul_(0.)
    ...
    def forward(
        ...
        return multi_head_attention_forward(query, key, value, 
                                            ...
                                            bias_v=self.bias_v,
                                            ma_module=self,
                                            #lrk_layers=[self.in_proj_left, self.in_proj_right, self.out_proj_left, self.out_proj_right],
