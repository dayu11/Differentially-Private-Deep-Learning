...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def use_batch_grad(self, scale=None):
        if(self.fc1_left.weight.grad == None):
            self.fc1_left.weight.grad = process_batch_grad(self.fc1_left.weight.batch_grad, scale=scale)#torch.sum(self.fc1_left.weight.batch_grad, dim=0)
            self.fc1_right.weight.grad = process_batch_grad(self.fc1_right.weight.batch_grad, scale=scale)#torch.sum(self.fc1_right.weight.batch_grad, dim=0)
            self.fc2_left.weight.grad = process_batch_grad(self.fc2_left.weight.batch_grad, scale=scale)#torch.sum(self.fc2_left.weight.batch_grad, dim=0)
            self.fc2_right.weight.grad = process_batch_grad(self.fc2_right.weight.batch_grad, scale=scale)#torch.sum(self.fc2_right.weight.batch_grad, dim=0)
        else:
            self.fc1_left.weight.grad += process_batch_grad(self.fc1_left.weight.batch_grad, scale=scale)#torch.sum(self.fc1_left.weight.batch_grad, dim=0)
            self.fc1_right.weight.grad += process_batch_grad(self.fc1_right.weight.batch_grad, scale=scale)#torch.sum(self.fc1_right.weight.batch_grad, dim=0)
            self.fc2_left.weight.grad += process_batch_grad(self.fc2_left.weight.batch_grad, scale=scale)#torch.sum(self.fc2_left.weight.batch_grad, dim=0)
    ...
    def assign_full_grad(self, linear_eval=False):
        ...
        self._assign_full_grad(self.fc1_left.weight, self.fc1_right.weight, self.fc1.weight, linear_eval=False)
        self._assign_full_grad(self.fc2_left.weight, self.fc2_right.weight, self.fc2.weight, linear_eval=False)
        
        self.fc1_left.weight.grad = None#.mul_(0)
        self.fc1_right.weight.grad = None#.mul_(0)
        self.fc2_left.weight.grad = None#.mul_(0)
        self.fc2_right.weight.grad = None#.mul_(0)
    ...
    def forward(
        ...
        if(self.is_training and self.need_decompose):
            ...
            fc1_left_data, fc1_right_data, fc1_residual = weight_decomposition(self.fc1.weight.data, self.args.rank, self.args.mode)
            fc2_left_data, fc2_right_data, fc2_residual = weight_decomposition(self.fc2.weight.data, self.args.rank, self.args.mode)
