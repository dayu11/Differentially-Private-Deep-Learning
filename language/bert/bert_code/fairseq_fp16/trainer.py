...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        #    )
        #)
        #for p in chain(self.model.named_parameters(), self.criterion.named_parameters()):
        #    print(p[0], p[1].shape.numel())
        #exit()
        #for p in self.model.parameters():
        #    p.requires_grad = False # we will assign differentially private gradient manually
        ...
        #self.full_params = []
        #if(self.args.sigma >= 0 or self.args.linear_eval):
            #params = []
            #lrk_params_flags = []
            for m in self.model.modules():
                if(hasattr(m, 'assign_full_grad')):
                    if(hasattr(m, 'in_proj_weight') and not self.args.no_attn and not self.args.linear_eval):
                        params.append(m.in_proj_weight)
                        params.append(m.in_proj_left.weight)
                        params.append(m.in_proj_right.weight)
                        params.append(m.out_proj.weight)
                        ...
                        #self.full_params += [m.in_proj_weight, m.out_proj.weight]
                    if(hasattr(m, 'fc1') and not self.args.linear_eval):
                        ...
                        params.append(m.fc2.weight)
                        params.append(m.fc2_left.weight)
                        params.append(m.fc2_right.weight)
                        lrk_params_flags += [False, True, True, False, True, True]
                        #self.full_params += [m.fc1.weight, m.fc2.weight]
                
                    # if(self.args.linear_eval):
                    
                    # elif(self.args.no_attn):
                    #     lrk_params_flags += [False, False, False, False, True, True]

        #if(self.args.sigma <= 0):
        #    new_params  = []
        #    for i, flag in enumerate(lrk_params_flags):
        #        if(not flag):
        ...
        elif(not self.args.linear_eval):
            for p in self.model.named_parameters():
        ...
        if self.args.fp16:
            try:
                ...
                raise ImportError("apex is required for fp16")
    ...
    def load_checkpoint(
        ...
        if os.path.exists(filename):
            ...
            try:
                missing_keys, _ = self.get_model().load_state_dict(state['model'], strict=False)
                if len(missing_keys) > 0:
                    print(
                        'Find missing keys when loading: {}'.format(missing_keys)
                    )
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
            ...
            try:
                with maybe_no_sync():
                    ...
                    if(self.args.sigma > 0):
                        ...
                        #input()
                        #print(sample_size, norms.cpu().numpy())
                        for m in self.model.modules():
                            if(hasattr(m, 'assign_full_grad')):
                                if(hasattr(m, 'in_proj_weight') and not self.args.no_attn and not self.args.linear_eval):
                        ...
                        if(classify_weight.grad == None):
                            classify_weight.grad = process_batch_grad(classify_weight.batch_grad, scale=scale)#torch.sum(classify_weight.batch_grad, dim=0)
                            classify_bias.grad = process_batch_grad(classify_bias.batch_grad, scale=scale)#torch.sum(classify_bias.batch_grad, dim=0)
                        else:
                            classify_weight.grad += process_batch_grad(classify_weight.batch_grad, scale=scale)#torch.sum(classify_weight.batch_grad, dim=0)
                            classify_bias.grad += process_batch_grad(classify_bias.batch_grad, scale=scale)#torch.sum(classify_bias.batch_grad, dim=0)
        ...
        if(not self.args.small_dataset):
            batch_size = self.args.max_sentences*update_freq
        else:
            batch_size = self.args.dataset_size
        ...
        if(self.args.sigma > 0):
            ...
            for i, p in enumerate(self.params):
                if(self.lrk_params_flags[i]):
                    p.grad /= batch_size#update_freq
                    sigma = self.args.sigma * self.args.clip
                    #print('before noise : grad norm: %.3f, sigma %.3f'%(p.grad.norm().item(), sigma/batch_size))
                    #if(sigma>0):
                    p.grad += torch.normal(0, sigma/batch_size, size=p.grad.shape).cuda().half()
                    #print('grad norm: %.3f, sigma %.3f'%(p.grad.norm().item(), sigma/batch_size))
                else:
                    p.grad = None
            #exit()
            #for pg in self.optimizer.fp32_params:
            #    print(pg.shape)
            #exit(
            for m in self.model.modules():
                if(hasattr(m, 'assign_full_grad')):
        ...
        try:
            ...
            skip = False
            if('MNLI' in self.args.sess or 'QQP' in self.args.sess or 'QNLI' in self.args.sess or 'SST-2' in self.args.sess):
                bs = self.args.max_sentences*self.args.update_freq[0]
                if(sample_size % bs != 0 and sample_size % bs < 0.95*bs):
            ...
            #for i, p in enumerate(self.params):
            #    if(not self.lrk_params_flags[i]):
